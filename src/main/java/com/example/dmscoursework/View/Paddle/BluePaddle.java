...
/**
 * The BluePaddle class inherits from Paddle, so it has access to all the same
...
public class BluePaddle extends Paddle {
    ...
    /**
     * Constructs a blue paddle whose upper-left corner is specified by
     ...
     */
    public BluePaddle(Point2D center, int width, int height, Rectangle container) {
    ...
    protected Rectangle makePaddle(int width, int height) {
        ...
        try {
            input = new FileInputStream(
                    "src/Assets/BlueAssets/BluePaddle.PNG");
