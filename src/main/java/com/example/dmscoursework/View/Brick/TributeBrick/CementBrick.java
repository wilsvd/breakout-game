...
/**
 * The CementBrick class inherits from Brick, so it has access
...
public class CementBrick extends Brick {
    ...
    /**
     * Constructs a cement brick whose upper-left corner is specified by the
    ...
    protected Shape makeBrick(Point2D pos, Dimension2D size) {
        setM_BrickShape(new Rectangle(pos.getX(), pos.getY(),
                size.getWidth() - 1,
                size.getHeight() - 1));

        GetM_BrickShape().setFill(DEF_INNER);
        GetM_BrickShape().setStroke(DEF_BORDER);
