...
/**
 * The FireTreeLevel is a level which takes multiple bricks and puts them
 * together into the shape of a tree.
 *
 *
...
public class FireTreeLevel {
    ...
    /**
     * Constructs a level looking like a fire tree that has 2 different types
     ...
     */
    public FireTreeLevel(LevelBuilder levelBuilder, int lineCount,
                         double brickSizeRatio,
                         int typeA, int typeB) {
        setM_LevelBuilder(levelBuilder);
        setM_TypeA(typeA);
        setM_TypeB(typeB);
        setLevel(makeTreeLevel(levelBuilder, lineCount, brickSizeRatio
        ));
    }

    /**
     * Makes a level that looks like a tree made from fire.
     *
     ...
     */
    private Brick[] makeTreeLevel(LevelBuilder levelBuilder,
        ...
        tmpBricks = makeTreeTop(tmpBricks, brickSize);
        tmpBricks = makeTreeStem(tmpBricks, brickSize);

        return tmpBricks;
    }

    /**
     * Makes the top of the tree.
     *
     * @param tmpBricks a Brick[], which is an array of bricks.
     * @param brickSize a Dimension2D, containing height and length of a brick.
     * @return a Brick[], which is an array of bricks.
     */
    private Brick[] makeTreeTop(Brick[] tmpBricks, Dimension2D brickSize) {
    ...
    /**
     * Makes the stem of the tree.
     *
     * @param tmpBricks a Brick[], which is an array of bricks.
     * @param brickSize a Dimension2D, containing height and length of a brick.
     * @return a Brick[], which is an array of bricks.
     */
    private Brick[] makeTreeStem(Brick[] tmpBricks, Dimension2D brickSize) {
        double brickHeight = brickSize.getHeight();
        double brickLength = brickSize.getWidth();
        setM_X(getM_X() + brickLength);
        int i = 0;
